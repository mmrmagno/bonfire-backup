...
import { Shield, Upload, Download, Clock, AlertTriangle, CheckCircle, Flame } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="ember-border rounded-lg p-6">
          ...
          <div className="space-y-2">
            {lastSync ? (
              <>
                <p className="text-sm text-green-400">{lastSync.toLocaleString()}</p>
                <p className="text-sm text-gray-400">Backup completed successfully</p>
              </>
            ) : (
              <>
      ...
      {isConfigured && (
        <div className="flex justify-center space-x-4">
          <button
            ...
            disabled={isLoading}
            className="px-8 py-3 bonfire-glow rounded-lg font-semibold text-orange-100 hover:shadow-xl hover:shadow-orange-500/30 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <div className="flex items-center space-x-2">
